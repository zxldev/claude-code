var oidc = (() => {
  ...
  var Be = (l, e) => {
      for (var t in e) ke(l, t, { get: e[t], enumerable: !0 })
    },
    ze = (l, e, t, r) => {
  ...
  var Qe = l => ze(ke({}, '__esModule', { value: !0 }), l)
  var gt = {}
  Be(gt, {
    AccessTokenEvents: () => Y,
    CheckSessionIFrame: () => Z,
    DPoPState: () => W,
    ErrorResponse: () => w,
    ErrorTimeout: () => E,
    InMemoryWebStorage: () => q,
    IndexedDbDPoPStore: () => ve,
    Log: () => Q,
    Logger: () => g,
    MetadataService: () => ee,
    OidcClient: () => ae,
    OidcClientSettingsStore: () => U,
    SessionMonitor: () => ie,
    SigninResponse: () => H,
    SigninState: () => D,
    SignoutResponse: () => re,
    ...
    User: () => L,
    UserManager: () => Te,
    UserManagerSettingsStore: () => ne,
    Version: () => qe,
    WebStorageStateStore: () => N,
  })
  var Ve = { debug: () => {}, info: () => {}, warn: () => {}, error: () => {} },
    x,
    R,
    Q = (s => (
      (s[(s.NONE = 0)] = 'NONE'),
  ...
  var g = class l {
    ...
    debug(...e) {
      x >= 4 && R.debug(l._format(this._name, this._method), ...e)
    }
    info(...e) {
      x >= 3 && R.info(l._format(this._name, this._method), ...e)
    }
    warn(...e) {
      x >= 2 && R.warn(l._format(this._name, this._method), ...e)
    }
    error(...e) {
      x >= 1 && R.error(l._format(this._name, this._method), ...e)
    }
    throw(e) {
    ...
    static debug(e, ...t) {
      x >= 4 && R.debug(l._format(e), ...t)
    }
    static info(e, ...t) {
      x >= 3 && R.info(l._format(e), ...t)
    }
    static warn(e, ...t) {
      x >= 2 && R.warn(l._format(e), ...t)
    }
    static error(e, ...t) {
      x >= 1 && R.error(l._format(e), ...t)
    }
  }
  Q.reset()
  ...
  function Ue(l, e) {
    ...
    let t = e.header === !0 ? 0 : 1,
      r = l.split('.')[t]
    if (typeof r != 'string')
      throw new V(`Invalid token specified: missing part #${t + 1}`)
    let i
    try {
      i = Xe(r)
    } catch (s) {
  ...
  var C = class {
    ...
    static async generateSignedJwt(e, t, r) {
      let i = m.encodeBase64Url(new TextEncoder().encode(JSON.stringify(e))),
        s = m.encodeBase64Url(new TextEncoder().encode(JSON.stringify(t))),
        n = `${i}.${s}`,
        o = await window.crypto.subtle.sign(
          { name: 'ECDSA', hash: { name: 'SHA-256' } },
          r,
          new TextEncoder().encode(n),
        ),
        a = m.encodeBase64Url(new Uint8Array(o))
      return `${n}.${a}`
    }
    static async generateSignedJwtWithHmac(e, t, r) {
      let i = m.encodeBase64Url(new TextEncoder().encode(JSON.stringify(e))),
        s = m.encodeBase64Url(new TextEncoder().encode(JSON.stringify(t))),
        n = `${i}.${s}`,
        o = await window.crypto.subtle.sign(
  ...
  var Ye = '10000000-1000-4000-8000-100000000000',
    Pe = l =>
      btoa([...new Uint8Array(l)].map(e => String.fromCharCode(e)).join('')),
    S = class S {
      static _randomWord() {
      ...
      static generateUUIDv4() {
        return Ye.replace(/[018]/g, t =>
          (+t ^ (S._randomWord() & (15 >> (+t / 4)))).toString(16),
        ).replace(/-/g, '')
      }
      static generateCodeVerifier() {
        return S.generateUUIDv4() + S.generateUUIDv4() + S.generateUUIDv4()
      }
      ...
      static generateBasicAuth(e, t) {
        let i = new TextEncoder().encode([e, t].join(':'))
        return Pe(i)
      }
      ...
      static async customCalculateJwkThumbprint(e) {
        ...
        switch (e.kty) {
          case 'RSA':
            t = { e: e.e, kty: e.kty, n: e.n }
            break
          case 'EC':
            t = { crv: e.crv, kty: e.kty, x: e.x, y: e.y }
            break
          case 'OKP':
            t = { crv: e.crv, kty: e.kty, x: e.x }
            break
          case 'oct':
            t = { crv: e.k, kty: e.kty }
            break
        ...
        let r = await S.hash('SHA-256', JSON.stringify(t))
        return S.encodeBase64Url(r)
      }
      static async generateDPoPProof({
        ...
        t &&
          ((n = await S.hash('SHA-256', t)),
          (o = S.encodeBase64Url(n)),
          (a.ath = o)),
          s && (a.nonce = s)
        try {
          let c = await crypto.subtle.exportKey('jwk', i.publicKey),
            d = {
              alg: 'ES256',
              typ: 'dpop+jwt',
              jwk: { crv: c.crv, kty: c.kty, x: c.x, y: c.y },
            }
          return await C.generateSignedJwt(d, a, i.privateKey)
        } catch (c) {
          throw c instanceof TypeError
            ? new Error(`Error exporting dpop public key: ${c.message}`)
            : c
        }
      }
      static async generateDPoPJkt(e) {
        try {
          let t = await crypto.subtle.exportKey('jwk', e.publicKey)
          return await S.customCalculateJwkThumbprint(t)
        } catch (t) {
          throw t instanceof TypeError
            ? new Error(
                `Could not retrieve dpop keys from storage: ${t.message}`,
              )
            : t
        }
      }
      ...
      static async generateClientAssertionJwt(e, t, r, i = 'HS256') {
        let s = Math.floor(Date.now() / 1e3),
          ...
          o = {
            ...
            sub: e,
            aud: r,
            jti: S.generateUUIDv4(),
            exp: s + 300,
            iat: s,
          },
          c = { HS256: 'SHA-256', HS384: 'SHA-384', HS512: 'SHA-512' }[i]
        if (!c)
          throw new Error(
            `Unsupported algorithm: ${i}. Supported algorithms are: HS256, HS384, HS512`,
          )
        ...
        return await C.generateSignedJwtWithHmac(n, o, u)
      }
    }
  S.encodeBase64Url = e =>
    Pe(e).replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_')
  var m = S
  var b = class {
    constructor(e) {
      this._name = e
  ...
  var f = class l extends b {
    constructor() {
      ...
      this._expiration = 0
      this._callback = () => {
        let t = this._expiration - l.getEpochTime()
        this._logger.debug('timer completes in', t),
          this._expiration <= l.getEpochTime() && (this.cancel(), super.raise())
      }
    }
    static getEpochTime() {
  ...
  var Y = class {
    constructor(e) {
      ...
      this._expiringTimer = new f('Access token expiring')
      this._expiredTimer = new f('Access token expired')
    ...
    async load(e) {
      ...
      if (e.access_token && e.expires_in !== void 0) {
        ...
        if ((t.debug('access token present, remaining duration:', r), r > 0)) {
          ...
          s <= 0 && (s = 1),
            t.debug('registering expiring timer, raising in', s, 'seconds'),
    ...
    async fetchWithTimeout(e, t = {}) {
      ...
      try {
        ...
        throw o instanceof DOMException && o.name === 'AbortError'
          ? new E('Network timed out')
          : o
      } finally {
        clearTimeout(n)
      }
    }
    async getJson(e, { token: t, credentials: r, timeoutInSeconds: i } = {}) {
      let s = this._logger.create('getJson'),
        n = { Accept: this._contentTypes.join(', ') }
      t &&
        (s.debug('token passed, setting Authorization header'),
        (n.Authorization = 'Bearer ' + t)),
        this._appendExtraHeaders(n)
      let o
      try {
        s.debug('url:', e),
        ...
        throw (s.error('Network Error'), d)
      }
      s.debug('HTTP response received, status', o.status)
      let a = o.headers.get('Content-Type')
      if (
        (a &&
          !this._contentTypes.find(d => a.startsWith(d)) &&
          s.throw(
            new Error(
              `Invalid response Content-Type: ${a != null ? a : 'undefined'}, from URL: ${e}`,
            ),
          ),
        o.ok &&
          this._jwtHandler &&
          a != null &&
          a.startsWith('application/jwt'))
      )
      ...
      try {
        ...
        throw (
          (s.error('Error parsing JSON response', d),
          o.ok ? d : new Error(`${o.statusText} (${o.status})`))
        )
      }
      if (!o.ok)
        throw (
          (s.error('Error from server:', c),
          c.error
            ? new w(c)
            : new Error(`${o.statusText} (${o.status}): ${JSON.stringify(c)}`))
        )
      return c
    }
    async postForm(
      ...
      let o = this._logger.create('postForm'),
        a = {
      ...
      try {
        ...
        throw (o.error('Network error'), h)
      }
      o.debug('HTTP response received, status', c.status)
      let d = c.headers.get('Content-Type')
      if (d && !this._contentTypes.find(h => d.startsWith(h)))
        throw new Error(
          `Invalid response Content-Type: ${d != null ? d : 'undefined'}, from URL: ${e}`,
        )
      let u = await c.text(),
        p = {}
      if (u)
        try {
          ...
          throw (
            (o.error('Error parsing JSON response', h),
            c.ok ? h : new Error(`${c.statusText} (${c.status})`))
          )
        }
      if (!c.ok) {
        if ((o.error('Error from server:', p), c.headers.has('dpop-nonce'))) {
          let h = c.headers.get('dpop-nonce')
          throw new X(h, `${JSON.stringify(p)}`)
        }
        throw p.error
          ? new w(p, t)
          : new Error(`${c.statusText} (${c.status}): ${JSON.stringify(p)}`)
      }
      return p
    }
    _appendExtraHeaders(e) {
      ...
      r.length !== 0 &&
        r.forEach(n => {
          ...
          if (s.includes(n.toLocaleLowerCase()) && Object.keys(e).includes(n)) {
            t.warn('Header could not be overridden', n, s)
            return
          }
          let o =
  ...
  var ee = class {
    ...
    async getMetadata() {
      ...
      return (
        ...
        (this._metadata = Object.assign({}, t, this._settings.metadataSeed)),
        this._metadata
      )
    }
    getIssuer() {
  ...
  var Ze = 'code',
    ...
    U = class {
      constructor({
        ...
        client_authentication: p = tt,
        token_endpoint_auth_signing_alg: h = 'HS256',
        prompt: v,
        display: j,
        max_age: J,
        ui_locales: K,
        ...
        loadUserInfo: _ = !1,
        requestTimeoutInSeconds: I,
        staleStateAgeInSeconds: O = rt,
        mergeClaimsStrategy: k = { array: 'replace' },
        disablePKCE: B = !1,
        stateStore: z,
        revokeTokenAdditionalContentTypes: Me,
        fetchRequestCredentials: Ee,
        refreshTokenAllowedScope: Ne,
        extraQueryParams: De = {},
        extraTokenParams: He = {},
        extraHeaders: We = {},
        dpop: Le,
        omitScopeWhenRequesting: je = !1,
      }) {
        var Ie
        if (
          ((this.authority = e),
          ...
          (this.token_endpoint_auth_signing_alg = h),
          (this.prompt = v),
          (this.display = j),
          (this.max_age = J),
          (this.ui_locales = K),
          ...
          (this.resource = F),
          (this.response_mode = $),
          (this.filterProtocolClaims = y != null ? y : !0),
          (this.loadUserInfo = !!_),
          (this.staleStateAgeInSeconds = O),
          (this.mergeClaimsStrategy = k),
          (this.omitScopeWhenRequesting = je),
          (this.disablePKCE = !!B),
          (this.revokeTokenAdditionalContentTypes = Me),
          (this.fetchRequestCredentials = Ee || 'same-origin'),
          (this.requestTimeoutInSeconds = I),
          z)
        )
          this.stateStore = z
        else {
          let Je = typeof window != 'undefined' ? window.localStorage : new q()
          this.stateStore = new N({ store: Je })
        }
        if (
          ((this.refreshTokenAllowedScope = Ne),
          (this.extraQueryParams = De),
          (this.extraTokenParams = He),
          (this.extraHeaders = We),
          (this.dpop = Le),
          this.dpop && !((Ie = this.dpop) != null && Ie.store))
        )
  ...
  var te = class {
    ...
    async exchangeCode({
      ...
      r || o.throw(new Error('A client_id is required')),
        t || o.throw(new Error('A redirect_uri is required')),
        n.code || o.throw(new Error('A code is required'))
      let a = new URLSearchParams({ grant_type: e, redirect_uri: t })
      for (let [p, h] of Object.entries(n)) h != null && a.set(p, h)
      if (
      ...
      let c,
        d = await this._metadataService.getTokenEndpoint(!1)
      switch (this._settings.client_authentication) {
        case 'client_secret_basic':
          c = m.generateBasicAuth(r, i)
          break
        case 'client_secret_post':
          a.append('client_id', r), i && a.append('client_secret', i)
          break
        case 'client_secret_jwt': {
          let p = await m.generateClientAssertionJwt(
            r,
            i,
            d,
            this._settings.token_endpoint_auth_signing_alg,
          )
          a.append('client_id', r),
            a.append(
              'client_assertion_type',
              'urn:ietf:params:oauth:client-assertion-type:jwt-bearer',
            ),
            a.append('client_assertion', p)
          break
        }
      }
      o.debug('got token endpoint')
      let u = await this._jsonService.postForm(d, {
        body: a,
        basicAuth: c,
        timeoutInSeconds: this._settings.requestTimeoutInSeconds,
    ...
    async exchangeCredentials({
      ...
      let n = this._logger.create('exchangeCredentials')
      ...
      for (let [u, p] of Object.entries(s)) p != null && o.set(u, p)
      if (
        (this._settings.client_authentication === 'client_secret_basic' ||
          this._settings.client_authentication === 'client_secret_jwt') &&
        r == null
      ...
      let a,
        c = await this._metadataService.getTokenEndpoint(!1)
      switch (this._settings.client_authentication) {
        case 'client_secret_basic':
          a = m.generateBasicAuth(t, r)
          break
        case 'client_secret_post':
          o.append('client_id', t), r && o.append('client_secret', r)
          break
        case 'client_secret_jwt': {
          let u = await m.generateClientAssertionJwt(
            t,
            r,
            c,
            this._settings.token_endpoint_auth_signing_alg,
          ...
          o.append('client_id', t),
            ...
            o.append('client_assertion', u)
          break
        }
      }
      n.debug('got token endpoint')
      let d = await this._jsonService.postForm(c, {
        body: o,
        basicAuth: a,
        timeoutInSeconds: this._settings.requestTimeoutInSeconds,
        initCredentials: this._settings.fetchRequestCredentials,
    ...
    async exchangeRefreshToken({
      ...
      t || o.throw(new Error('A client_id is required')),
        n.refresh_token || o.throw(new Error('A refresh_token is required'))
      let a = new URLSearchParams({ grant_type: e })
      for (let [p, h] of Object.entries(n))
        Array.isArray(h)
          ? h.forEach(v => a.append(p, v))
          : h != null && a.set(p, h)
      if (
        (this._settings.client_authentication === 'client_secret_basic' ||
          this._settings.client_authentication === 'client_secret_jwt') &&
    ...
    async revoke(e) {
      ...
      t.debug(
        `got revocation endpoint, revoking ${((s = e.token_type_hint)) != null ? s : 'default token type'}`,
      )
      ...
      i.set('client_id', this._settings.client_id),
        ...
        await this._jsonService.postForm(r, {
          body: i,
          timeoutInSeconds: this._settings.requestTimeoutInSeconds,
        }),
  ...
  var ge = class {
    ...
    async validateSigninResponse(e, t, r) {
      ...
      this._processSigninState(e, t),
        ...
        e.isOpenId && this._validateIdTokenAttributes(e, '', t.nonce),
    ...
    async validateCredentialsResponse(e, t) {
      let r = this._logger.create('validateCredentialsResponse'),
        i = e.isOpenId && !!e.id_token
      i && this._validateIdTokenAttributes(e),
        r.debug('tokens validated'),
        await this._processClaims(e, t, i),
    ...
    async validateRefreshResponse(e, t) {
      ...
      ;(e.userState = t.data),
        (s = e.session_state) != null || (e.session_state = t.session_state),
    ...
    validateSignoutResponse(e, t) {
      ...
      if (
        ...
        throw (r.warn('Response was error', e.error), new w(e))
    }
    ...
    async _processClaims(e, t = !1, r = !0) {
      ...
      i.debug('user info claims received from user info endpoint'),
        ...
        (e.profile = this._claimsService.mergeClaims(
          e.profile,
          this._claimsService.filterProtocolClaims(s),
        )),
    ...
    async _processCode(e, t, r) {
      ...
      if (e.code) {
        ...
        let s = await this._tokenClient.exchangeCode({
    ...
    _validateIdTokenAttributes(e, t, r) {
      var n
      let i = this._logger.create('_validateIdTokenAttributes')
      i.debug('decoding ID Token JWT')
      let s = C.decode((n = e.id_token) != null ? n : '')
      if (
        (s.sub || i.throw(new Error('ID Token is missing a subject claim')),
        r &&
          s.nonce !== r &&
          i.throw(
            new Error(
        ...
        let o = C.decode(t)
        s.sub !== o.sub &&
          i.throw(new Error('sub in id_token does not match current sub')),
          s.auth_time &&
            s.auth_time !== o.auth_time &&
            i.throw(
              new Error(
          ...
          s.azp &&
            s.azp !== o.azp &&
            i.throw(new Error('azp in id_token does not match original azp')),
          !s.azp &&
            o.azp &&
            i.throw(
              new Error(
  ...
  var P = class l {
    constructor(e) {
      ;(this.id = e.id || m.generateUUIDv4()),
        (this.data = e.data),
        e.created && e.created > 0
          ? (this.created = e.created)
          : (this.created = f.getEpochTime()),
    ...
    static fromStorageString(e) {
      return (
        g.createStatic('State', 'fromStorageString'),
        Promise.resolve(new l(JSON.parse(e)))
      )
    }
    static async clearStaleState(e, t) {
      ...
      for (let n = 0; n < s.length; n++) {
        ...
        if (a)
          try {
            let d = await l.fromStorageString(a)
            r.debug('got item from key:', o, d.created),
              d.created <= i && (c = !0)
          } catch (d) {
            r.error('Error parsing state for key:', o, d), (c = !0)
          }
        else r.debug('no item in storage for key:', o), (c = !0)
        c && (r.debug('removed item for key:', o), e.remove(o))
      }
    }
  }
  var D = class l extends P {
    constructor(e) {
      super(e),
    ...
    static async create(e) {
      let t =
          e.code_verifier === !0
            ? m.generateCodeVerifier()
            : e.code_verifier || void 0,
        r = t ? await m.generateCodeChallenge(t) : void 0
      return new l({ ...e, code_verifier: t, code_challenge: r })
    }
    toStorageString() {
      return (
  ...
  var ue = class ue {
    ...
    static async create({
      ...
      disablePKCE: K,
      dpopJkt: A,
      omitScopeWhenRequesting: F,
      ...$
    }) {
      if (!e)
        throw (ue._logger.error('create: No url passed'), new Error('url'))
      if (!r)
        throw (
          (ue._logger.error('create: No client_id passed'),
          new Error('client_id'))
        )
      if (!i)
        throw (
          (ue._logger.error('create: No redirect_uri passed'),
          new Error('redirect_uri'))
        )
      if (!s)
        throw (
          (ue._logger.error('create: No response_type passed'),
          new Error('response_type'))
        )
      if (!n)
        throw (ue._logger.error('create: No scope passed'), new Error('scope'))
      if (!t)
        throw (
          (ue._logger.error('create: No authority passed'),
          new Error('authority'))
        )
      let y = await D.create({
          data: o,
          request_type: c,
          url_state: p,
          code_verifier: !K,
        ...
        _ = new URL(e)
      _.searchParams.append('client_id', r),
        _.searchParams.append('redirect_uri', i),
        _.searchParams.append('response_type', s),
        F || _.searchParams.append('scope', n),
        u && _.searchParams.append('nonce', u),
        A && _.searchParams.append('dpop_jkt', A)
      let I = y.id
      p && (I = `${I}${T}${p}`),
        _.searchParams.append('state', I),
        y.code_challenge &&
          (_.searchParams.append('code_challenge', y.code_challenge),
          _.searchParams.append('code_challenge_method', 'S256')),
        h &&
          (Array.isArray(h) ? h : [h]).forEach(k =>
            _.searchParams.append('resource', k),
          )
      for (let [O, k] of Object.entries({ response_mode: a, ...$, ...j }))
        k != null && _.searchParams.append(O, k.toString())
      return new ue({ url: _.href, state: y })
    }
  }
  ue._logger = new g('SigninRequest')
  var pe = ue
  var it = 'openid',
    H = class {
      constructor(e) {
        ...
        if (
          ...
          let t = decodeURIComponent(this.state).split(T)
          ;(this.state = t[0]),
            t.length > 1 && (this.url_state = t.slice(1).join(T))
        }
      ...
      set expires_in(e) {
        typeof e == 'string' && (e = Number(e)),
          e !== void 0 &&
            e >= 0 &&
            (this.expires_at = Math.floor(e) + f.getEpochTime())
      }
      get isOpenId() {
        var e
        return (
          ((e = this.scope) == null ? void 0 : e.split(' ').includes(it)) ||
          !!this.id_token
        )
      }
    }
  var he = class {
    constructor({
      ...
      if (
        ...
        i && (c.searchParams.append('post_logout_redirect_uri', i), t || a))
      ) {
        this.state = new P({ data: t, request_type: n, url_state: a })
        let d = this.state.id
        a && (d = `${d}${T}${a}`), c.searchParams.append('state', d)
      }
      for (let [d, u] of Object.entries({ ...s }))
        u != null && c.searchParams.append(d, u.toString())
      this.url = c.href
    }
  }
  var re = class {
  ...
  var st = ['nbf', 'jti', 'auth_time', 'nonce', 'acr', 'amr', 'azp', 'at_hash'],
    nt = ['sub', 'iss', 'aud', 'exp', 'iat'],
    me = class {
      constructor(e) {
        this._settings = e
      ...
      filterProtocolClaims(e) {
        ...
        if (this._settings.filterProtocolClaims) {
          ...
          Array.isArray(this._settings.filterProtocolClaims)
            ...
            : (r = st)
          for (let i of r) nt.includes(i) || delete t[i]
        }
        return t
      }
      mergeClaims(e, t) {
        ...
        for (let [i, s] of Object.entries(t))
          if (r[i] !== s)
            if (Array.isArray(r[i]) || Array.isArray(s))
              if (this._settings.mergeClaimsStrategy.array == 'replace')
                r[i] = s
              else {
                let n = Array.isArray(r[i]) ? r[i] : [r[i]]
                for (let o of Array.isArray(s) ? s : [s])
                  n.includes(o) || n.push(o)
                r[i] = n
              }
            else
              typeof r[i] == 'object' && typeof s == 'object'
                ? (r[i] = this.mergeClaims(r[i], s))
                : (r[i] = s)
        return r
      }
    }
  ...
  var ae = class {
    constructor(e, t) {
      ...
      ;(this.settings = e instanceof U ? e : new U(e)),
        (this.metadataService = t != null ? t : new ee(this.settings)),
        (this._claimsService = new me(this.settings)),
        (this._validator = new ge(
          this.settings,
          this.metadataService,
          this._claimsService,
        )),
        (this._tokenClient = new te(this.settings, this.metadataService))
    }
    async createSigninRequest({
      ...
      display: v = this.settings.display,
      max_age: j = this.settings.max_age,
      ui_locales: J = this.settings.ui_locales,
      acr_values: K = this.settings.acr_values,
      ...
      dpopJkt: _,
      omitScopeWhenRequesting: I = this.settings.omitScopeWhenRequesting,
    }) {
      let O = this._logger.create('createSigninRequest')
      if (d !== 'code')
        throw new Error(
          'Only the Authorization Code flow (with PKCE) is supported',
        )
      let k = await this.metadataService.getAuthorizationEndpoint()
      O.debug('Received authorization endpoint', k)
      let B = await pe.create({
        url: k,
    ...
    async readSigninResponseState(e, t = !1) {
      let r = this._logger.create('readSigninResponseState'),
        i = new H(G.readParams(e, this.settings.response_mode))
      if (!i.state) throw (r.throw(new Error('No state in response')), null)
      let s = await this.settings.stateStore[t ? 'remove' : 'get'](i.state)
      if (!s)
        throw (r.throw(new Error('No matching state found in storage')), null)
      return { state: await D.fromStorageString(s), response: i }
    }
    async processSigninResponse(e, t, r = !0) {
      let i = this._logger.create('processSigninResponse'),
        { state: s, response: n } = await this.readSigninResponseState(e, r)
      if (
        (i.debug('received state from storage; validating response'),
        this.settings.dpop && this.settings.dpop.store)
      ) {
        let o = await this.getDpopProof(this.settings.dpop.store)
        t = { ...t, DPoP: o }
      }
      try {
        await this._validator.validateSigninResponse(n, s, t)
      } catch (o) {
        if (o instanceof X && this.settings.dpop) {
          let a = await this.getDpopProof(this.settings.dpop.store, o.nonce)
          ;(t.DPoP = a), await this._validator.validateSigninResponse(n, s, t)
        } else throw o
      }
      return n
    }
    async getDpopProof(e, t) {
      ...
      return (
        (await e.getAllKeys()).includes(this.settings.client_id)
          ? ((i = await e.get(this.settings.client_id)),
            i.nonce !== t &&
              t &&
              ((i.nonce = t), await e.set(this.settings.client_id, i)))
          : ((r = await m.generateDPoPKeys()),
            (i = new W(r, t)),
            await e.set(this.settings.client_id, i)),
        await m.generateDPoPProof({
    ...
    async processResourceOwnerPasswordCredentials({
      ...
      let s = await this._tokenClient.exchangeCredentials({
          username: e,
          password: t,
          ...i,
        }),
        n = new H(new URLSearchParams())
      return (
        Object.assign(n, s),
        await this._validator.validateCredentialsResponse(n, r),
        n
      )
    }
    async useRefreshToken({
      ...
      var u
      let o = this._logger.create('useRefreshToken'),
        a
      if (this.settings.refreshTokenAllowedScope === void 0) a = e.scope
      else {
        let p = this.settings.refreshTokenAllowedScope.split(' ')
        a = (((u = e.scope) == null ? void 0 : u.split(' ')) || [])
          .filter(v => p.includes(v))
          .join(' ')
      }
      if (this.settings.dpop && this.settings.dpop.store) {
        let p = await this.getDpopProof(this.settings.dpop.store)
        s = { ...s, DPoP: p }
      }
      let c
      try {
        ...
        if (p instanceof X && this.settings.dpop)
          (s.DPoP = await this.getDpopProof(this.settings.dpop.store, p.nonce)),
            (c = await this._tokenClient.exchangeRefreshToken({
              refresh_token: e.refresh_token,
              scope: a,
              redirect_uri: t,
      ...
      let d = new H(new URLSearchParams())
      return (
        Object.assign(d, c),
        o.debug('validating response', d),
        await this._validator.validateRefreshResponse(d, { ...e, scope: a }),
        d
      )
    }
    async createSignoutRequest({
      ...
      return (
        u &&
          (a.debug('Signout request has state to persist'),
          await this.settings.stateStore.set(u.id, u.toStorageString())),
        d
      )
    }
    async readSignoutResponseState(e, t = !1) {
      let r = this._logger.create('readSignoutResponseState'),
        i = new re(G.readParams(e, this.settings.response_mode))
      if (!i.state) {
        if ((r.debug('No state in response'), i.error))
          throw (r.warn('Response was error:', i.error), new w(i))
        return { state: void 0, response: i }
      }
      let s = await this.settings.stateStore[t ? 'remove' : 'get'](i.state)
      if (!s)
        throw (r.throw(new Error('No matching state found in storage')), null)
      return { state: await P.fromStorageString(s), response: i }
    }
        ...
        try {
          ...
          if (i) {
            ...
            let s = this._userManager.settings.client_id,
              n = this._userManager.settings.checkSessionIntervalInSeconds,
              o = this._userManager.settings.stopCheckSessionOnError,
              a = new Z(this._callback, s, i, n, o)
            await a.load(), (this._checkSessionIFrame = a), a.start(t)
  ...
  var L = class l {
    ...
    get expires_in() {
      if (this.expires_at !== void 0) return this.expires_at - f.getEpochTime()
    }
    set expires_in(e) {
      e !== void 0 && (this.expires_at = Math.floor(e) + f.getEpochTime())
    }
    get expired() {
    ...
    static fromStorageString(e) {
      return g.createStatic('User', 'fromStorageString'), new l(JSON.parse(e))
    }
  }
  var Ae = 'oidc-client',
    se = class {
      constructor() {
        this._abort = new b('Window navigation aborted')
      ...
      async navigate(e) {
        ...
        let { url: r, keepOpen: i } = await new Promise((s, n) => {
          let o = c => {
            var p
            let d = c.data,
              u = (p = e.scriptOrigin) != null ? p : window.location.origin
            if (!(c.origin !== u || (d == null ? void 0 : d.source) !== Ae)) {
              try {
                let h = G.readParams(d.url, e.response_mode).get('state')
                if (
                  (h || t.warn('no state found in response url'),
                  c.source !== this._window && h !== e.state)
                )
                  return
              } catch {
          ...
          let a = new BroadcastChannel(`oidc-client-popup-${e.state}`)
          a.addEventListener('message', o, !1),
            this._disposeHandlers.add(() => a.close()),
            this._disposeHandlers.add(
      ...
      static _notifyParent(e, t, r = !1, i = window.location.origin) {
        let s = { source: Ae, url: t, keepOpen: r },
          n = new g('_notifyParent')
        if (e)
          n.debug('With parent. Using parent.postMessage.'), e.postMessage(s, i)
        else {
          n.debug('No parent. Using BroadcastChannel.')
          let o = new URL(t).searchParams.get('state')
          if (!o)
            throw new Error(
              "No parent and no state in URL. Can't complete notification.",
            )
          let a = new BroadcastChannel(`oidc-client-popup-${o}`)
    ...
    ne = class extends U {
      constructor(e) {
        let {
          ...
          silentRequestTimeoutInSeconds: p,
          automaticSilentRenew: h = !0,
          validateSubOnSilentRenew: v = !0,
          includeIdTokenInSilentRenew: j = !1,
          monitorSession: J = !1,
          monitorAnonymousSession: K = !1,
          checkSessionIntervalInSeconds: A = at,
          ...
          accessTokenExpiringNotificationTimeInSeconds: O = ot,
          maxSilentRenewTimeoutRetries: k,
          userStore: B,
        } = e
        if (
          (super(e),
          ...
          (this.silentRequestTimeoutInSeconds = p || d || Ce),
          (this.automaticSilentRenew = h),
          (this.validateSubOnSilentRenew = v),
          (this.includeIdTokenInSilentRenew = j),
          (this.monitorSession = J),
          (this.monitorAnonymousSession = K),
          (this.checkSessionIntervalInSeconds = A),
          (this.stopCheckSessionOnError = $),
          (this.query_status_response_type = F),
          (this.revokeTokenTypes = y),
          (this.revokeTokensOnSignout = _),
          (this.includeIdTokenInSilentSignout = I),
          (this.accessTokenExpiringNotificationTimeInSeconds = O),
          (this.maxSilentRenewTimeoutRetries = k),
          B)
        )
          this.userStore = B
        else {
          let z = typeof window != 'undefined' ? window.sessionStorage : new q()
          this.userStore = new N({ store: z })
        }
      }
    }
  var ce = class l extends se {
    constructor({ silentRequestTimeoutInSeconds: t = Ce }) {
      super()
      this._logger = new g('IFrameWindow')
      ;(this._timeoutInSeconds = t),
        (this._frame = l.createHiddenIframe()),
    ...
    async navigate(t) {
      ...
      let r = setTimeout(() => {
        this._abort.raise(new E('IFrame timed out without a response'))
    ...
    static notifyParent(t, r) {
      return se._notifyParent(window.parent, t, !1, r)
    }
  }
  var _e = class {
    constructor(e) {
    ...
    async prepare({
      ...
      return new ce({ silentRequestTimeoutInSeconds: e })
    }
    async callback(e) {
      this._logger.create('callback'),
        ce.notifyParent(e, this._settings.iframeNotifyParentOrigin)
    }
  }
  var ct = 500,
    lt = 1e3,
    le = class extends se {
      constructor({
        ...
        let n = oe.center({ ...xe, ...r })
        ;(this._window = window.open(void 0, t, oe.serialize(n))),
          (this.abortOnClose = !!s),
          i &&
            i.addEventListener('abort', () => {
              var o
              this._abort.raise(
                new Error((o = i.reason) != null ? o : 'Popup aborted'),
              )
            }),
          r.closePopupWindowAfterInSeconds &&
            ...
            setTimeout(() => {
              ...
              this.close()
            }, r.closePopupWindowAfterInSeconds * lt)
      }
      async navigate(t) {
        var s
        ;(s = this._window) == null || s.focus()
        let r = setInterval(() => {
            ;(!this._window || this._window.closed) &&
              (this._logger.debug(
                'Popup closed by user or isolated by redirect',
              ),
              i(),
              this._disposeHandlers.delete(i),
              this.abortOnClose &&
                this._abort.raise(new Error('Popup closed by user')))
          }, ct),
          i = () => clearInterval(r)
        return this._disposeHandlers.add(i), await super.navigate(t)
      }
      close() {
        this._window &&
          (this._window.closed ||
      ...
      static notifyOpener(t, r) {
        se._notifyParent(window.opener, t, r),
          !r && !window.opener && window.close()
      }
    }
  var fe = class {
    constructor(e) {
  ...
  var Se = class extends Y {
    constructor(t) {
      ...
      this._userLoaded = new b('User loaded')
      this._userUnloaded = new b('User unloaded')
      this._silentRenewError = new b('Silent renew error')
      this._userSignedIn = new b('User signed in')
      this._userSignedOut = new b('User signed out')
      this._userSessionChanged = new b('User session changed')
    }
    async load(t, r = !0) {
      await super.load(t), r && (await this._userLoaded.raise(t))
    }
    async unload() {
  ...
  var be = class {
    constructor(e) {
      ...
      this._isStarted = !1
      this._retryTimer = new f('Retry Silent Renew')
      ...
      this._tokenExpiring = async () => {
        ...
        try {
          ...
          if (t instanceof E) {
            ...
            let r = this._userManager.settings.maxSilentRenewTimeoutRetries
            if (r !== void 0 && this._timeoutRetryCount > r) {
              e.error(
                `Timeout retry limit reached (${this._timeoutRetryCount} > ${r}), raising silentRenewError:`,
                t,
              ),
                (this._timeoutRetryCount = 0),
                await this._userManager.events._raiseSilentRenewError(t)
              return
            }
            e.warn(
  ...
  var Te = class {
    constructor(e, t, r, i) {
      ...
      ;(this.settings = new ne(e)),
        (this._client = new ae(e)),
        (this._redirectNavigator = t != null ? t : new we(this.settings)),
        (this._popupNavigator = r != null ? r : new fe(this.settings)),
        (this._iframeNavigator = i != null ? i : new _e(this.settings)),
        (this._events = new Se(this.settings)),
        (this._silentRenewService = new be(this)),
        this.settings.automaticSilentRenew && this.startSilentRenew(),
        (this._sessionMonitor = null),
        this.settings.monitorSession && (this._sessionMonitor = new ie(this))
    }
    get events() {
      return this._events
    }
    ...
    async getUser(e = !1) {
      let t = this._logger.create('getUser'),
        r = await this._loadUser()
      return r
        ? (t.info('user loaded'), await this._events.load(r, e), r)
        : (t.info('user not found in storage'), null)
    }
    async removeUser() {
    ...
    async signinRedirect(e = {}) {
      ...
      let { redirectMethod: t, ...r } = e,
        i
      ;(n = this.settings.dpop) != null &&
        n.bind_authorization_code &&
        (i = await this.generateDPoPJkt(this.settings.dpop))
      let s = await this._redirectNavigator.prepare({ redirectMethod: t })
      await this._signinStart({ request_type: 'si:r', dpopJkt: i, ...r }, s)
    }
    ...
    async signinResourceOwnerCredentials({
      ...
      let i = this._logger.create('signinResourceOwnerCredential'),
        s = await this._client.processResourceOwnerPasswordCredentials({
          username: e,
          password: t,
          skipUserInfo: r,
      ...
      let n = await this._buildUser(s)
      return (
        n.profile && n.profile.sub
          ? i.info('success, signed in subject', n.profile.sub)
          : i.info('no subject'),
        n
      )
    }
    async signinPopup(e = {}) {
      ...
      let t = this._logger.create('signinPopup'),
        r
      ;(p = this.settings.dpop) != null &&
        p.bind_authorization_code &&
        (r = await this.generateDPoPJkt(this.settings.dpop))
      let {
      ...
      c || t.throw(new Error('No popup_redirect_uri configured'))
      let d = await this._popupNavigator.prepare({
          popupWindowFeatures: i,
          popupWindowTarget: s,
          popupSignal: n,
          popupAbortOnClose: o,
        }),
        u = await this._signin(
          {
            request_type: 'si:p',
            redirect_uri: c,
            display: 'popup',
      ...
      return (
        u &&
          (u.profile && u.profile.sub
            ? t.info('success, signed in subject', u.profile.sub)
            : t.info('no subject')),
        u
      )
    }
    ...
    async signinSilent(e = {}) {
      ...
      if (!e.forceIframeAuth && s != null && s.refresh_token) {
        ...
        return await this._useRefreshToken({
          ...
          resource: i.resource,
          extraTokenParams: i.extraTokenParams,
          timeoutInSeconds: r,
        })
      }
      let n
      ;(d = this.settings.dpop) != null &&
        d.bind_authorization_code &&
        (n = await this.generateDPoPJkt(this.settings.dpop))
    ...
    async querySessionStatus(e = {}) {
      ...
      try {
        let c = {},
          d = await this._client.processSigninResponse(a.url, c)
        return (
          t.debug('got signin response'),
          d.session_state && d.profile.sub
            ? (t.info('success for subject', d.profile.sub),
              { session_state: d.session_state, sub: d.profile.sub })
            : (t.info('success, user not authenticated'), null)
        )
      } catch (c) {
        if (this.settings.monitorAnonymousSession && c instanceof w)
          switch (c.error) {
            case 'login_required':
    ...
    async _signinEnd(e, t) {
      let r = this._logger.create('_signinEnd'),
        i = {},
        s = await this._client.processSigninResponse(e, i)
      ...
      if (t) {
        if (t !== i.profile.sub)
          throw (
            ...
            new w({ ...e, error: 'login_required' }))
          )
        r.debug('current user matches user returned from signin')
      }
      return (
        await this.storeUser(i),
        r.debug('user stored'),
        await this._events.load(i),
        i
      )
    }
    ...
    async _signoutStart(e = {}, t) {
      ...
      try {
        ...
        r.debug('loaded current user from storage'),
          this.settings.revokeTokensOnSignout && (await this._revokeInternal(s))
    ...
    async signoutSilent(e = {}) {
      ...
      let t = this._logger.create('signoutSilent'),
        { silentRequestTimeoutInSeconds: r, ...i } = e,
        s = this.settings.includeIdTokenInSilentSignout
          ? (a = await this._loadUser()) == null
            ? void 0
            : a.id_token
          : void 0,
    ...
    async _loadUser() {
      ...
      return t
        ? (e.debug('user storageString loaded'), L.fromStorageString(t))
    ...
    async dpopProof(e, t, r, i) {
      var n, o
      let s = await ((o =
        (n = this.settings.dpop) == null ? void 0 : n.store) == null
        ? void 0
        : o.get(this.settings.client_id))
      if (s)
        return await m.generateDPoPProof({
          url: e,
    ...
    async generateDPoPJkt(e) {
      ...
      if (!t) {
        let r = await m.generateDPoPKeys()
        ;(t = new W(r)), await e.store.set(this.settings.client_id, t)
      }
      return await m.generateDPoPJkt(t.keys)
    }
  }
  var Oe = '3.5.0'
  var qe = Oe
  var ve = class {
    ...
    async set(e, t) {
      await (await this.createStore(this._dbName, this._storeName))(
        'readwrite',
        i => (i.put(t, e), this.promisifyRequest(i.transaction)),
      )
    }
    async get(e) {
      return await (await this.createStore(this._dbName, this._storeName))(
        'readonly',
        r => this.promisifyRequest(r.get(e)),
      )
    }
    async remove(e) {
      ...
      return (
        await (await this.createStore(this._dbName, this._storeName))(
          'readwrite',
          i => this.promisifyRequest(i.delete(e)),
        ),
        t
      )
    }
    async getAllKeys() {
      return await (await this.createStore(this._dbName, this._storeName))(
        'readonly',
        t => this.promisifyRequest(t.getAllKeys()),
      )
    }
    promisifyRequest(e) {
    ...
    async createStore(e, t) {
      ...
      let i = await this.promisifyRequest(r)
      return async (s, n) => {
        let a = i.transaction(t, s).objectStore(t)
        return await n(a)
      }
    }
  }
